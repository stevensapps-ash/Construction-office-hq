...
const items=[["Dashboard",LayoutDashboard],["Secretary",Users],["Contacts",Users],["Customers",Users],["Projects",FolderKanban],["Project Details",FileText],["Progress",LayoutDashboard],["Activity",CalendarDays],["Files",FolderKanban],["Documents",FileText],["Checklist",NotebookPen],["Estimates",FileText],["Invoices",Receipt],["Contracts",ClipboardSignature],["Change Orders",FileText],["Approvals",ClipboardSignature],["Blueprints",Ruler],["Permits",FileText],["Receipts",Receipt],["Schedule",CalendarDays],["Notes",NotebookPen],["AI Credits",Receipt],["Banking",Receipt],["Settings",Settings]] as const;
export default function Home(){const [active,setActive]=useState("Dashboard");return <main><aside><div className="brand"><div className="mark"><HardHat size={25}/></div><div><b>CONSTRUCTION</b><span>OFFICE HQ</span></div></div><nav aria-label="Main navigation">{items.map(([n,I])=><button type="button" key={n} aria-current={active===n?"page":undefined} className={active===n?"active":""} onClick={()=>setActive(n)}><I size={19}/>{n}</button>)}</nav></aside><section><header><div><p>BUSINESS COMMAND CENTER</p><h1>{active}</h1></div><div className="actions"><GlobalSearch go={setActive}/><button className="primary" onClick={()=>setActive("Customers")}><Plus size={18}/> New Project</button></div></header>{active==="Secretary"?<Secretary/>:active==="Contacts"?<ContactBook/>:active==="Customers"?<CustomerManager/>:active==="Projects"?<ProjectManager/>:active==="Project Details"?<ProjectDetails/>:active==="Progress"?<ProjectProgress/>:active==="Activity"?<ActivityLog/>:active==="Files"?<FileCabinet/>:active==="Documents"?<DocumentCenter/>:active==="Checklist"?<JobChecklist/>:active==="Estimates"?<EstimateBuilder/>:active==="Invoices"?<InvoiceManager/>:active==="Contracts"?<ContractManager/>:active==="Change Orders"?<ChangeOrderManager/>:active==="Approvals"?<ApprovalManager/>:active==="Blueprints"?<BlueprintManager/>:active==="Permits"?<PermitManager/>:active==="Receipts"?<ReceiptManager/>:active==="Schedule"?<ScheduleManager/>:active==="Notes"?<NotesManager/>:active==="AI Credits"?<AICredits/>:active==="Banking"?<BankingManager/>:active==="Settings"?<SettingsPanel/>:active==="Dashboard"?<DashboardLive go={setActive}/>:<ComingSoon name={active}/>}</section></main>}
